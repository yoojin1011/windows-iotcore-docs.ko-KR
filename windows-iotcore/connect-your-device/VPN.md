...
---
# <a name="leveraging-vpn-capabilities-for-your-windows-10-iot-core-device"></a>Windows 10 IoT Core 장치에 대 한 VPN 기능을 활용 하 여

Windows 10 IoT Core VPN 기능을 활용 하려면 아래 지침을 따르세요.

> [!NOTE]
> 대부분의 아래 지침에 맞게 조정 해야 합니다. 사용자가 연결 되어 있는 VPN 호스트에 특정 됩니다. 인증서 사용은 예제입니다.

## <a name="establishing-a-vpn-connection"></a>VPN 연결 

1. 필요한 인증서를 구하여 IoT 장치 (예: \vpntest 폴더)에 복사 합니다.

* RASTest.pfx
* IssuingCA.crl
* RootCA.crl

2. 로컬 컴퓨터 인증서를 적용 한 합니다. 관리자 권한으로 장치에 대 한 PowerShell
...
3. 사용자 인증서를 적용 한 합니다. SSH를 사용 하 여 "DefaultAccount"으로 IoT 장치에 로그인 합니다.
b. 명령 프롬프트에서 "PowerShell"을 입력 합니다.
다. ("기본 계정"으로 로그인) 하는 동안 PowerShell에서 다음 명령을 실행 합니다.
...
4. 수정 호스트 파일 c:\windows\system32\driverS\etc\hosts 파일 (예: 아래;)에 항목 추가

> |    |    |    |
> |----|----| ---|
> | 10.10.10.10 | MyVPN.DomainName.org | 필요에 따라 IP 주소 및 도메인 이름으로 대체 |

5. 빌드 VPN 테스트 응용 프로그램 소스 코드에서 "MyVPN.DomainName.org" 대체 합니다. 필요에 따라 추가 보강 합니다.

6. 아래에 코드 배포는 Windows 10 IoT 장치에 "시작 및 중지 하는 VPN 연결" 섹션.
임의의 "프로필 이름을"를 입력 하 고 "VPN에 연결" 단추를 누릅니다. 


## <a name="starting-and-stopping-a-vpn-connection"></a>시작 하 고 VPN 연결을 중지 합니다.

아래 코드를 사용 하 여 시작 하 고 VPN 연결을 중지 합니다.
