...
---
# <a name="release-notes-for-build-16267"></a>빌드 16267에 대 한 릴리스 정보

_빌드 번호 16267. 8 월 2017_
...
* 일반 버그 수정 
* Joule의 Intel EOL (수명 종료)으로 인해 Joule에 대 한 FFUs는 곧 중단 될 예정입니다. RS3 RTM에 대해 Joule이 릴리스되지 않습니다. 고객은 지원 되는 다른 Soc [정보](../../learn-about-hardware/SoCsAndCustomBoards.md) 중 하나를 사용 하 여 대체 플랫폼을 식별 해야 합니다.
* IOT_WEBB_EXTN 기능이 등록 기능을 제거 하도록 리팩터링 되었습니다. 이제 온 보 딩 기능을 IOT_ONBOARDING_APP로 사용할 수 있습니다.
이 업데이트를 사용 하면 온 보 딩 기능이 제거 되 고 등록 기능을 사용 하는 사용자는 장치를 다시 플래시 하 여이 기능을 다시 가져와야 합니다.
* 이제 다중 언어 패키지를 사용할 수 있습니다. 여러 언어를 지 원하는 FFUs를 만들 수 있습니다. 자세한 내용은 [MultiLangSample](https://github.com/ms-iot/iot-adk-addonkit/tree/16299/Source-arm/Products/MultiLangSample) 및 [SingleLangSample](https://github.com/ms-iot/iot-adk-addonkit/tree/16299/Source-arm/Products/SingleLangSample) 를 참조 하세요.
* IoTSettings .exe를 사용 하 여 시스템 언어를 변경할 수 있습니다. 자세한 내용은 언어 구성 섹션 [에서 확인할 수](https://developer.microsoft.com/en-us/windows/iot/docs/cortanaoniotcore) 있습니다.

## <a name="additional-information"></a>추가 정보
* 드래곤 보드 이미지에 사용 되는 BSP 버전은 2112.0.0.0입니다. 


## <a name="known-issues-in-this-build"></a>이 빌드의 알려진 이슈:
* 스토어에서 게시 된 앱은 Microsoft에서 서명 하지 않는 한 Windows를 통해 exe를 호출할 수 없습니다.
* JavaScript 앱에 대 한 Visual Studio 디버깅이 작동 하지 않습니다. Visual Studio에서 디버거를 사용 하지 않고 앱을 배포 하 고 시작할 수 있습니다.
Visual Studio에서 앱을 배포 하 고 장치 포털 또는 iotstartup에서 시작할 수도 있습니다.
* MBM (MinnowBoard Max) 펌웨어 버전 0.93에서 네트워크 연결에 실패 했습니다. 펌웨어 버전 0.94에서 수정 되었습니다. 
* Visual Studio에서 배포하는 F5 드라이버는 IoT Core에서 작동하지 않습니다. 
* InstallOemCerts 명령 오류로 인해 이미지를 만들 수 없습니다.
* 백그라운드 작업은 Windows 장치 포털의 앱 관리자 보기 (WDP)에 표시 되지 않습니다.
...
Raspberry Pi 3에는 bluetooth를 등록 하기 위해 다른 동글을 사용 하지 않도록 설정 해야 하는 온보드 bluetooth가 있습니다. `telnet/ssh session` 를 열고를 실행 합니다.`reg add hklm\system\controlset001\services\BtwSerialH5Bus /v Start /t REG_DWORD /d 4 `
다음 명령을 사용 하 여 WiFi를 사용 하지 않도록 설정할 수 있습니다.`reg add hklm\system\controlset001\services\bcmsdh43xx /v Start /t REG_DWORD /d 4`
...
하드웨어 가속을 사용 하도록 설정 하는 그래픽 드라이버는 Joule 이미지에 포함 되어 있지 않습니다. 시스템은 성능 및 CPU 사용률에 영향을 주는 소프트웨어 렌더링을 사용 합니다. 

### <a name="for-minnowboard"></a>MinnowBoard
#### <a name="minnowboard-max-boot-and-firmware-update"></a>Minnowboard Max 부팅 및 펌웨어 업데이트 
MinnowBoard Max는 펌웨어가 .092 이상이어야만 부팅됩니다. 최소 권장 펌웨어 버전은 “MinnowBoard MAX 0.92 32-Bit”입니다. 펌웨어 업데이트는에서 다운로드할 수 있습니다.  
...
```
예를 들어  `checknetisolation loopbackexempt -a -n=IoTOnboardingTask-uwp_1w720vyc4ccym 
Reboot`
...
일부 USB 키보드 및 마우스는 IoT Core에서 작동하지 않을 수 있습니다. 다른 키보드 또는 마우스를 사용하세요. 유효성이 검사 된 주변 장치의 목록은 [여기의 설명서](../../learn-about-hardware/HardwareCompatList.md)에서 확인할 수 있습니다.
...
다음 코드 줄이 충돌할 `BackgroundMediaPlayer.MessageReceivedFromForeground += OnMessageReceivedFromForeground;`수 있습니다. 충돌을 방지 하려면 먼저 실행 되도록이 코드를 추가 합니다.`var player = BackgroundMediaPlayer.Current;`
...
1) 디바이스의 명령줄(예: SSH, Powershell)`w32tm /config /syncfromflags:manual /manualpeerlist:"0.windows.time.com 1.pool.ntp.org 2.something else, ...`
