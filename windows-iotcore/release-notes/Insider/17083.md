...
---
# <a name="release-notes-for-build-17083"></a>빌드 17083에 대 한 릴리스 정보
_빌드 번호 17083. 2 월 2018_
...
* 일반 버그 수정 
* IO와이어 AP 이미지에서 플래시 모드를 사용 하도록 설정 했습니다.


## <a name="additional-information"></a>추가 정보
* 드래곤 보드 이미지에 사용 되는 BSP 버전은 2116.0.0.0입니다. 
...
*   마이크가 작동 하지 않습니다.
*   마우스 포인터는 드래곤 보드 이미지에 표시 되지 않습니다.
*   응용 프로그램을 헤드리스 모드로 배포할 수 없습니다. 헤드리스 모드에서 장치가 작동 하지 않습니다.
*   F5 응용 프로그램의 응용 프로그램 배포는 첫 번째 시도에서 작동 하지 않습니다. 두 번째 시도가 성공 합니다.
*   UWP 시작 앱은 IoT Core 기본 앱으로 대체 됩니다.
...
모니터 연결이 끊어진 경우 Raspberry Pi가 디스플레이 해상도를 유지하지 않을 수 있습니다. 모니터의 EDID는 모니터가 연결될 때 시스템의 해상도를 설정하는 데 사용됩니다.  
...
MinnowBoard Max는 펌웨어가 .092 이상이어야만 부팅됩니다. 최소 권장 펌웨어 버전은 "MinnowBoard MAX 0.92 64-Bit"입니다. 펌웨어 업데이트는 [여기](https://firmware.intel.com/projects/minnowboard-max)서 다운로드할 수 있습니다.
...
예: checknetisolation loopbackexempt-a-n = Is\on보드 Ingtask-uwp_1w720vyc4ccym Reboot. 
...
* [이미지에 파일 및 레지스트리 설정 추가](https://docs.microsoft.com/windows-hardware/manufacture/iot/add-a-registry-setting-to-an-image)

### <a name="starting-the-ftp-server"></a>FTP 서버 시작 
시작 시 FTP 서버가 더 이상 기본적으로 실행되지 않음 

한 번만 실행하려면: 
* SSH\PS로 로그인하여 다음 명령으로 FTP를 시작합니다.  
  ```
  start ftpd.exe 
  ```

부팅 때마다 실행하려면 사용자가 스케줄러 작업을 만들어야 합니다. 
* SSH\PS로 로그인하여 스케줄러 작업을 만듭니다. 
* schtasks/create/tn "IoTFTPD"/tr ftpd/ru 시스템/sc onstart 
